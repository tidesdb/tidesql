...
CREATE TABLE sbtest%d(
  ...
  %s (id)
   ...
   -- Batch size for inserts (to avoid TDB_ERR_TOO_LARGE with TidesDB)
   local batch_size = 1000
   local c_val
   local pad_val

   for batch_start = 1, sysbench.opt.table_size, batch_size do
      local batch_end = math.min(batch_start + batch_size - 1, sysbench.opt.table_size)
...
-- Partition keys by thread to reduce conflicts in MVCC databases like TidesDB
-- Each thread gets a non-overlapping range of keys
local function get_id()
   local thread_id = sysbench.tid or 0
   local num_threads = sysbench.opt.threads or 1
   local range_size = math.floor(sysbench.opt.table_size / num_threads)
   local range_start = thread_id * range_size + 1
   local range_end = (thread_id + 1) * range_size
   if thread_id == num_threads - 1 then
      range_end = sysbench.opt.table_size  -- Last thread gets remainder
   end
   return sysbench.rand.uniform(range_start, range_end)
end

-- Original get_id for read operations (can access any key)
local function get_id_global()
...
function execute_point_selects()
   ...
   for i = 1, sysbench.opt.point_selects do
      param[tnum].point_selects[1]:set(get_id_global())  -- Reads can access any key
...
local function execute_range(key)
   ...
   for i = 1, sysbench.opt[key] do
      local id = get_id_global()  -- Reads can access any key
